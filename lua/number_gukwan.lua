...
local function decimal_func(str, posMap, valMap)
    ...
    valMap = valMap or { [0] = "零", "壹", "貳", "參", "肆", "伍", "陸", "柒", "捌", "玖" }
...
local function formatNum(num, t)
    ...
    if tonumber(t) < 1 then
        ...
        wordFigure = { "零", "壹", "貳", "參", "肆", "伍", "陸", "柒", "捌", "玖" }
    end
...
local function number2cnChar(num, flag, digitUnit, wordFigure) --flag=0中文小写反之为大写
    ...
    if tonumber(flag) < 1 then
        ...
        wordFigure = wordFigure or { [1] = "〇", [2] = "一", [3] = "十", [4] = "圓" }
    else
        digitUnit = digitUnit or { [1] = "萬", [2] = "億" }
        wordFigure = wordFigure or { [1] = "零", [2] = "壹", [3] = "拾", [4] = "圓" }
    end
    local lens = string.len(num)
    if lens < 5 then
    ...
    if result ~= "" then result = result .. wordFigure[4] else result = "數值超限！" end
...
local function number_translatorFunc(num)
    ...
    table.insert(result,
        ...
        decimal_func(numberPart.dec, { [1] = "角", [2] = "分", [3] = "厘", [4] = "毫" },
            { [0] = "〇", "一", "二", "三", "四", "五", "六", "七", "八", "九" }), "〔金額小寫〕" })

    local number2cnCharInt = number2cnChar(numberPart.int, 1)
    local number2cnCharDec = decimal_func(numberPart.dec, { [1] = "角", [2] = "分", [3] = "厘", [4] = "毫" }, { [0] = "零", "壹", "貳", "參", "肆", "伍", "陸", "柒", "捌", "玖" })
    if string.len(numberPart.int) > 4 and number2cnCharInt:find('^拾[壹貳參肆伍陸柒捌玖]?') and number2cnCharInt:find('[萬億]')  then -- 简易地规避 utf8 匹配问题
        local number2cnCharInt_var = number2cnCharInt:gsub('^拾', '壹拾')
        table.insert(result, { number2cnCharInt_var .. number2cnCharDec , "〔金額大寫〕"})
        -- 会计书写要求 https://github.com/iDvel/rime-ice/issues/989
    else
        table.insert(result, { number2cnCharInt .. number2cnCharDec , "〔金額大寫〕"})
    end
    return result
end
