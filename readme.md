...
以下描述以小欖話爲主，並其同廣州話異同。調值方音有異，此處以小欖作爲標準。由於本人不擅聲調，調值或有誤。下列以**老派音**爲準，如無提及不考慮白讀、訓讀、例外音。

雖有參攷《中山方言誌》同《中山市誌》，但其描述並不仔細，對音韻地位亦並無準確描述。

### 聲母

- 區分平翹
  - 同大多數粵語一般，沙田方言無平翹之分。（菊韻平翹默認爲濟南型）

- 區分日以
  - 同珠三角多數粵語一般，沙田方言無日以之分。

- 區分來孃
  - 孃母／泥母（n）爲鼻音，以鼻發音。來母則爲流音，以喉發音。沙田方言多數不能區分，無論來孃皆讀流音（l）。如「男」`naam4`讀作「藍」`laam4`、「農」`nung4`讀作「龍」`lung4`。
  - 此現象亦見於許多珠三角方言，例如新派廣州話，並被稱之爲懶音。
- 區分透曉
  - 透母（t）爲齒音送氣，曉母（h）爲喉音送氣。僅小欖地區部份人無法區分，皆讀作喉音送氣（h）。如「貪」`taam1`讀作`haam1`、「偸」`tau1`讀作`hau1`。
  - 不知爲何《中山方言誌》並無指出此現象。
- 區分疑影
  - 疑母（ng）爲喉鼻音，影母／亞母（0）今時則指零聲母。有部份方音／人士無法分辨，例如將本應爲零聲母字「鴨」`aap3`讀作`ngaap3`。亦或是喉鼻音字「牙」`ngaa4` 讀作`aa4`。
...
- 止攝開口三等字
  - 此類字情況較爲複雜，廣州話除`z`、`c`、`s`接`i`之外，其他皆已裂化，接`ei`。而沙田話更加複雜：
    - 古精莊組聲母者（即師韻），則讀作`yu`（少數方音同廣州讀`i`）。如「資」`zy1`「師」`sy1`「字」`zy6`「此」`cy2`等。
      - 例外爲「四」「死」小韻，多讀作`i`或`ei`。
      - 近年受廣州影響，不少人將師韻字讀成`i`。
...
- 蟹攝合口一等字
  - 廣州話多數讀`ui`。疑母兩地無異皆讀`oi`，如「外」`ngoi6`。但廣州話讀`eoy`者，小欖話則多讀作`yu`。
...
- 「ooi」音
  - 此音僅見於蟹攝三四等字同止攝合口銳音，此類字於廣州以及多數沙田方言中多讀作`eoy`。如「雖」`seoy1`「歲」`seoy3`「蕊」`jeoy5`「銳」`jeoy6`。
    - 根據《中山方言誌》，有少數方音保持古音`ooi`並保持同`oi`既對立，即「隨」`cooi4 /tʃʰoi/`「才」`coi4 /tʃʰɔi/`不同音。亦有少數方音爲同一音位，即「隨」「才」`coi4`同音。
      - 如「隨」一字，`cooi4`（罕）、`coi4`（罕）、`ceoy4`三種可能。
      - 讀`eoy`者通常亦有白讀音`oi`。如「迎娶（文）」`jing4 ceoy4`「娶親（白）」`coi4 can1`，「隨心（文）」`ceoy4 sam1`「隨意（白）」`coi4 ji3`。
- 遇攝字
  ...
  - 遇攝合口一等字基本上變化同廣州類似。
    ...
    - 小欖話則裂化爲`au`，如「度」`dau6`「無」`mau4`「做（作）」`zau6`「粗」`cau1`。
      - 根據《中山方言誌》，有方音裂化同廣州類似，裂化爲`ou`。少數方音則發生低化，讀`o`。如「賭」一字，則有`dau2`，`dou2`，`do2`（罕）三種可能。（注意「都」白讀`do1`並非來自於此，`do1`來自於其訓讀「多」）
...
- 江攝宕攝字
  ...
  - 根據《中山方言誌》，有部份方言有四種對立，故菊韻亦分四種以便轉換：`ong`、`oong`、`oeng`、`wong`，即「江」`gong1`「崗」`goong1`「薑」`goeng1`「光」`gwong1`四種。
- 效攝一等字
  - 廣州話中效攝一等字讀`ou`，但小欖則發生元音融合，讀`o`。如「高」`go1`「刀」`do1`「毛」`mo4`「草」`co2`。
  - 根據《中山方言誌》，有方音低化爲`au`、同廣州音一樣爲`ou`。亦有方音元音融合之後依舊保持同`o`既對立，即「高」`goo1 /go/`「哥」`go1 /gɔ/`不同音。
- 咸攝見系字
  - 沙田話中依舊保留`om`同`am`既對立，即「柑」`gom1 /gɔm/`「金」`gam1 /gɐm/`不同音。常見字有「鴿」`gop3`「合」`hop6`「敢」`gom2`。粵字有「搇（覆蓋）」`kom2`「噉（「個物」合音）」`gom3`等。但此對立已不見於新派音，新派多數將`om/op`讀作`am/ap`。
- 白讀音
  - 沙田話白讀音較廣州豐富。
    - 如效攝白讀，廣州話僅有「掉」`deu6`一字，而小欖話有「敲」`heu1`「覺」`geu3`「貓」`meu1`，「炒」`ceu2`等。
    - 如山攝白讀，廣州話並無白讀，而小欖話有「刮」`gwet3`「閂（名詞）」`soet1`「閂（動詞）」`soen1`「關」`gwen1`「挽」`wen5`；「邊」`ben1`「煙」`en1/jen1`「癲」`den1`「見」`gen3`等。
    - 如咸攝白讀，廣州話僅有「夾」`gep3`一字，而小欖話有「減」`gem2`「斬」`zem2`「插」`cep3`「餡」`hem2`等。
    - 臻攝亦有白讀，「順氣（白）」`san6 hi3`。另有其他如「多」`doe1`「頂」`doeng2`等。
- ak讀成aak（小欖話）
  - 此現象可確認，尤其老派，新派則相混。
...
1. 聲調至多爲十個（含特殊調）。陰聲爲古清音，陽聲爲古濁音。平上去入皆有，陰入分上陰入下陰入，對應不同韻母，再加特殊調「2*」。下以小欖話爲例：
2. 同廣州一樣，陰平有高平（55）高降（53）兩種。（加筆：根據《中山方言誌》，多個次方言中，不止陰平，陰上、陽平、陰入皆有兩種調值）
3. 「2*」並非如廣州只見於入聲字變調（如「雀」`zoek2 /tʃœk35/`、「賊」`caak2 /tʃʰak35/`等），亦常見於陽平字變調，同部分字白讀（「子」白讀`zaai2* /tʃai35/`）、「臘味」`laap6 mei2* /lap2 mei35/`就同本身陰上調值（24）不一。此調亦用作表達過去、完成之文法意味，如「食飯」讀`sik6 faan6 /sik21 faːn21/`則無過去意味，多爲名詞或動詞(eat)。讀`sik2* faan6 /ʃik35 fan35`則表達動作已完成。（爲免麻煩本方案不再另立。）
4. 陰去本歸陽平，皆讀42，如「睏覺」讀`fan2* geu4 /fɐn35 kɛu42`/（過去・完成）。但受廣州話影響，多數情況下陰去同廣州一樣讀若33。
5. 陰平接陽平時發生變調。首字讀若穗音陰去，次字則爲陰平，如「蕃薯」則讀作`faan3 sy1 /fan33 ʃy55*/`（亦可連續陰平），如「窗簾」`coeng1 lim1 /tʃʰœŋ53 lim55/`。
6. 亦有不少陽平字白讀爲陰平，如「錢」`cin1 /tʃin53/`「鎚」`ceoy1 /tʃʰɵy55/`「切」`cit1 /tʃʰit5/`「丸」`jyn1 /jyn53/`。
