...
<img src="pic\gamjam1.png" />

<img src="pic\gamjam2.png"/>

聲調同粵拼無異，下面會詳細說明。古代音請參攷[菊韻拼音表 – なかやま園](https://zonsan.fc2.page/?p=1583)，其他可參攷[順德系方言特徵 – なかやま園](https://zonsan.fc2.page/?p=1580)。
...
- 「江」韻母讀/oong/ [oŋ]，對應廣州話 /ong/，故可直接用粵拼現有`ong`。
- 「鋼」韻母讀/yoong/ [yoŋ]，廣州話無對應音位，故使用擴展粵拼`oong`。爲打字方便，`ong`亦可使用。
- 「薑」韻母讀/yong/ [yɔŋ]，對應廣州話 /oeng/，故可直接用粵拼現有`oeng`。而擴展粵拼`iong`亦可使用。
- 「廣」韻母讀/woong/ [woŋ]，對應廣州話 /wong/，故可直接用粵拼現有`wong`。

以橫欄六沙音爲例子，其「ooi音」（即古蟹攝三等合口字同止攝合口銳音）仍有一定對立：

- 【遇開三】「趣」韻母裂化讀/eoy/ [øy]，對應廣州話 /eoy/，故可直接用粵拼現有`eoi`。
- 【蟹開一】「菜」韻母讀/oi/ [ʊi]，對應廣州話 /oi/，故可直接用粵拼現有`oi`。
- 【蟹合三】「脆」韻母讀/ooy/ [oø]，廣州話無對應音位，故使用擴展粵拼`ooi`。

### `gu/ku`同`gwu/kwu`

【以下僅限擁有圓脣聲母方言】

雖然絕大多數字典同記音不分辨`gu/ku`同`gwu/kwu`，但兩者實際並非同音位。`gwu`同`kwu`最爲常見，多數字都讀作圓脣而非展脣。而`gu`同`ku`一般爲擬聲擬態詞所用，相對少見。

亦有部份方言將效攝讀作`u`，聲母則爲展脣。即「高」讀`gu1`而「姑」讀`gwu1`。菊韻目前無此類方言。
...
2\*並不如廣州話「新入」，只見於入聲字變調，如小欖話「子」白讀`zaai2*`就同陰上調值（通常爲 24，而特殊調多爲 35）不一。此調亦用作表達過去、完成之文法意味。超高陰平只出現於陰平連續變調。爲免麻煩，特殊調打字同 2（陰上）通用。 

1*出於變調，小欖話有，其他方言未確認。

### 零聲母

包括小欖話以內部份方言在細音存在零聲母同以母對立。如「拎」`ing1`同「英」`jing1`。

### 聲調

<img src="pic\gamjam3.png"/>

聲調同調值小欖話爲準，共有11個聲調（含特殊調），部份方言僅有 9 個或 8 個聲調（含特殊調），香山系方言只有 6 個甚至更少。 

菊韻不處理特殊調。

其中陰平(高降)同上陰入(口)（或可歸爲特殊調）大致爲口語字詞。以下簡單介紹特殊調：

- 特殊調「1*」
  - 口語變：甩牙（lat1* ngaa4，牙齒脫落），棘（kik1*，頭、腦、頭痛）
  - 連續變：窗簾（coeng1* lim1），蕉蕾（ziu1* leoy1）
  - 文法：熄掣（sik1* zai3，已經關閉開關）
- 特殊調「2\*」：
  - 口語變：坐月（co5 jyut2\*），雀（zoek2*），男仔（naam4 zaai2\*）
  - 文法：食飯（sik2* faan6，已經食飯）
...
- 順德系——順德市
  ...
  - `gukwan-gwanon`【均安話】：以均安話比較有代表性特徵製作，兼容部份新派發音。
- 香山系——中山市
...
- 見組止攝開口三等字未裂化讀作`i`而非`ei`，如「其」`ki4`「起」`hi2`「機」`gi1`「紀」`gi3`等。而陳村則未裂化。
...
- 【補充】部份方言曉匣喻細音字仍讀`h`而非`j`，如「爺」`he4`「賢」`hin4`「雨」`hy5`「藥」`hoek6`。
...
- 以下字較爲共通，且對輸入影響不大，全部收錄在主字庫，不再另立。
  ...
  - 「應」「拎」「煙」等字仍讀零聲母（此種白讀少見，而香山系方言老派無論文白皆讀零聲母）
...
- `gukwan-naamtau-daisaa.schema.yaml`：南頭低沙音
  - 以下以排坎音爲基底講述。下爲重要特徵：
    ...
    - 廣州話聲母`w`同韻腹`u/o`結合者，排坎聲母則爲`f`，如「黃」`fong4`「換」`fun6`「會」`fui6`「活」`fut6`。
...
- `gukwan-wongpou.schema.yaml`：黃圃音
  ...
  - 下爲鎮二同三社已知區別：
    ...
    - 廣州話聲母`w`同韻腹`u/o`結合者，陳村聲母則爲`f`，如「黃」`fong4`「換」`fun6`「會」`fui6`「活」`fut6`。
...
- `gukwan-longmong-kwanon.schema.yaml`：浪網群安音
  - 本方案根據浪網兩個記音點製作。下爲重要特徵：
    ...
    - 【群安】廣州話聲母w同韻腹u/o結合者，群安聲母則爲f，如「黃」`fong4`「換」`fun6`「會」`fui6`「活」`fut6`。
      - 此類字往往越接近現代，就會有越多字跟廣州一樣讀`w`。如已不能分辨者，請將 42~44 行`xform`改爲`derive`。
    - 「花」「輝」「快」「筷」等少部分曉母合口字讀`w-`
...
- `gukwan-daailoeng.schema.yaml`：大良音
  - 本方案根據大良話代表特徵製作。下爲重要特徵：
    ...
    - 曉匣喻細音字讀`h`而非`j`，如「爺」`he4`「賢」`hin4`「雨」`hy5`「藥」`hoek6`。
      - 此類字往往越接近現代，就會有越多字跟廣州一樣讀`j`。例如小欖就基本剩低極少數白讀字。如已不能分辨者，請將 37 行井號刪除，竝將 37~40 行`xform`改爲`derive`。
    - 廣州話聲母`w`同韻腹`u/o`結合者，大良聲母則爲`f`，如「黃」`fong4`「換」`fun6`「會」`fui6`「活」`fut6`。
      - 此類字往往越接近現代，就會有越多字跟廣州一樣讀`w`。如已不能分辨者，請將 42~44 行`xform`改爲`derive`。
    - 「花」「輝」「快」「筷」等少部分曉母合口字讀`w-`
...
- `gukwan-cancyn.schema.yaml`：陳村音
  - 本方案根據陳村話代表特徵製作。下爲重要特徵：
    - 不分來孃且疑母脫落，故零聲母可通疑母字：如「我」`ngo5`可打`o5`），「硬」`ngaang6`可打`aang6`。
    - 不分`ak`同`aak`，故`aak`音可通ak：如「勒」`lak6`可打`laak6`，「得」`dak1`可打`daak1`。老派只有`aak`，無`ak`。
...
- `gukwan-gwanaan.schema.yaml`：均安音
  - 下爲均安話重要特徵：
    - 不分來孃且疑母脫落，故零聲母可通疑母字：如「我」`ngo5`可打`o5`），「硬」`ngaang6`可打`aang6`。
    - 不分`ak`同`aak`，故可以 aak 音打 ak（如「勒」`lak6`可打`laak6`）。老派只有`aak`，無`ak`
    - 效攝一等字讀`aau`，如「高」`gaau1`「刀」`daau1`「掃」`saau3`「毛」`maau4`。
    - 山攝開口一等字陽聲韻見系不讀`on`讀`aan`，如「漢」`haan3`「岸」`ngaan6`「安」`aan1`「乾」`gaan1`，但入聲中新派同廣州話一樣，只有少數作爲白讀字保留，如「割」`gaat3`「渴」`haat3`。
    ...
    - 廣州話聲母`w`同韻腹`u/o`結合者，均安聲母則爲`f`，如「黃」`fong4`「換」`fun6`「會」`fui6`「活」`fut6`。
      - 此類字往往越接近現代，就會有越多字跟廣州一樣讀`w`。如已不能分辨者，請將 42~44 行`xform`改爲`derive`。
      - 均安有部份`hw`不變`f`或`w`，而係變`h`，如「慌」`hong1`「往」`hong5`等。<mark>由於已懶得整詞庫，此類字須手動設置。</mark>
    - 「火」「過」讀`u`。<mark>由於已懶得整詞庫，此類字須手動設置。</mark>
    - 「我」「個」等白讀，順德話一般讀`-oi`，均安則讀`-aai`。<mark>由於已懶得整詞庫，此類字須手動設置。</mark>
    - 「花」「輝」「快」「筷」等少部分曉母合口字讀`w-`

## 香山系方言

香山系方言今時勢微，主要分佈在石岐鎮（今石歧街道）、南區（環城）、東區、張家邊（今中山港街道），石歧周邊地區亦有分佈。中山以外，珠海部份地區，如唐家、金<ruby>鼎<rp>(</rp><rt>deng2</rt><rp>)</rp></ruby>、前山、南屏亦有分佈。澳門曾經亦是講香山系方言，由於歷史原因，今時絕大多數人都操廣州話。
...
- 韻母
  - 假攝三等字有嚴重混讀現象，有部份的而且保留介音讀`ia`，如「椰」`jaa4`「爺」`jaa4`「夜」`jaa3`「惹」`jaa3`（日母字），但部份字卻跟周邊其他方言讀`e`，如「社」`se2`「借」`ze3`。（或只有以`j`爲聲母者存古）
